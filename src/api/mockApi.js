...
const USERS = {
  'priya.new@tata.com': {
    ...
    project_ids: ['proj_bil'], avatar: 'PP',
  },
  'arjun.old@tata.com': {
...
const PROJECTS = [
  { id: 'proj_bil', name: 'BillingOS', code: 'BIL', status: 'active', description: 'Billing and invoicing platform', repo_url: 'github.com/tata/billing-os' },
  { id: 'proj_pay', name: 'PaymentAPI', code: 'PAY', status: 'active', description: 'Payment gateway integration layer', repo_url: 'github.com/tata/payment-api' },
  { id: 'proj_cpf', name: 'CorePlatform', code: 'CPF', status: 'active', description: 'Shared platform services', repo_url: 'github.com/tata/core-platform' },
]
...
const EXTRACTIONS = [
  {
    problem_summary: 'Monthly billing export produces a blank CSV file with no error message',
    root_cause: 'ERP system has a hard row limit of 500 on any export. Exceeding this causes a silent failure — the file comes out empty.',
    solution_steps: [
      'Open the ERP export panel and set the date range as usual.',
    ...
    tags: ['ERP', 'billing', 'export'],
  },
  {
    problem_summary: 'Race condition in the async payment pipeline causes duplicate charges',
    root_cause: 'Two concurrent webhook events trigger the same payment handler without a mutex lock.',
    solution_steps: [
      'Add a Redis distributed lock keyed on payment_id.',
      'Set the lock TTL to 30 seconds.',
      'Check the lock before processing each webhook event.',
    ],
    tags: ['async', 'payments', 'race-condition'],
  },
  {
    problem_summary: 'REDIS_URL missing from .env.example breaks new developer setup',
...
const doCapture = async (method, payload, ms) => {
  ...
  return {
    ...
    transcription: method === 'audio' ? 'So I just fixed that export issue again. The monthly export was giving us a blank CSV…' : null,
...
const ENTRIES = [
  {
    id: 'ENT-0041', project_id: 'proj_bil', employee: 'Arjun Sharma', team: 'Backend',
    problem_summary: 'Monthly billing export produces a blank CSV file',
    root_cause: 'ERP row limit of 500 causes a silent export failure — the file comes out empty.',
    solution_steps: ['Set the export row limit to 400', 'Run two batches with offset', 'Merge the CSV files'],
    tags: ['bug', 'ERP', 'billing'], source_type: 'audio', code_capture_subtype: null,
    confidence_score: 0.91, frequency_seen: 11, verification_status: 'verified', capture_date: '2026-05-18',
  },
  {
    id: 'ENT-0039', project_id: 'proj_pay', employee: 'Arjun Sharma', team: 'Backend',
    problem_summary: 'Race condition in async payment pipeline causes duplicate charges',
    root_cause: 'Two concurrent webhook events trigger the same payment handler without a mutex lock.',
    solution_steps: ['Add Redis distributed lock on payment_id', 'Set TTL to 30 seconds', 'Check lock before processing'],
    tags: ['bug', 'async', 'payments'], source_type: 'audio', code_capture_subtype: null,
    confidence_score: 0.94, frequency_seen: 4, verification_status: 'verified', capture_date: '2026-05-12',
  },
  {
    id: 'ENT-0037', project_id: 'proj_bil', employee: 'Arjun Sharma', team: 'Backend',
  ...
  {
    id: 'ENT-0035', project_id: 'proj_cpf', employee: 'Sneha Gupta', team: 'Platform',
    problem_summary: 'Database migrations must run before code deploy or the app crashes on startup',
    root_cause: 'New code references columns that do not exist until the migration runs.',
    solution_steps: ['Run the migrate command first', 'Wait for confirmation', 'Then deploy the new code', 'Rollback order is the reverse'],
    tags: ['deployment', 'database'], source_type: 'vtt', code_capture_subtype: null,
    confidence_score: 0.89, frequency_seen: 8, verification_status: 'verified', capture_date: '2026-05-01',
  },
  {
    id: 'ENT-0033', project_id: 'proj_pay', employee: 'Sneha Gupta', team: 'Platform',
    problem_summary: 'Auth token real expiry is 55 minutes, not 60 as documented',
    root_cause: 'The vendor silently reduced the TTL. Five minutes of clock skew causes unexpected logouts.',
  ...
  {
    id: 'ENT-0031', project_id: 'proj_bil', employee: 'Arjun Sharma', team: 'Backend',
    problem_summary: 'Do not touch the legacy billing module — it causes silent data corruption',
    root_cause: 'The legacy module has undocumented side effects on shared database state. A refactor was attempted and abandoned.',
    solution_steps: ['Never modify files under /src/legacy/billing/', 'Create a wrapper if new behaviour is needed', 'Talk to Arjun before any changes'],
    tags: ['architecture', 'tech-debt'], source_type: 'code', code_capture_subtype: 'code_comment',
    repo_name: 'billing-os', file_path: 'src/legacy/billing/core.py',
    confidence_score: 0.96, frequency_seen: 2, verification_status: 'verified', capture_date: '2026-04-20',
  },
  {
    id: 'ENT-0029', project_id: 'proj_cpf', employee: 'Sneha Gupta', team: 'Platform',
    problem_summary: 'Memory leak in the file upload handler crashes the server after ~200 uploads',
  ...
  {
    id: 'ENT-0027', project_id: 'proj_bil', employee: 'Arjun Sharma', team: 'Backend',
    problem_summary: 'Intentional auth bypass on the internal health check route',
    root_cause: 'The health check endpoint skips JWT validation by design so monitoring tools can poll it without credentials.',
    ...
    repo_name: 'billing-os', file_path: 'src/api/health.py',
...
const ANSWERS = [
  {
    answer: 'This has happened before — the blank CSV comes from the ERP\u2019s hard row limit of 500. When billing volume crosses that at month end, the export silently fails. The fix: split the export into two batches of 400 rows each, then merge them. This issue has come up 11 times.',
    match: ['csv', 'export', 'blank', 'billing', 'empty'],
    sources: [
      { entry_id: 'ENT-0041', project_id: 'proj_bil', problem_summary: 'Monthly billing export produces a blank CSV file', source_type: 'audio', team: 'Backend', confidence_score: 0.91, frequency_seen: 11, verification_status: 'verified', similarity: 0.89, capture_date: '2026-05-18', employee: 'Arjun Sharma' },
    ],
  },
  {
    answer: 'Migrations always run before the code deploy. New code references columns that don\u2019t exist until the migration runs — deploy code first and the app crashes on startup. Rollback order is the reverse: revert the migration first, then the code.',
    match: ['migration', 'deploy', 'deployment', 'order', 'rollback'],
    sources: [
      { entry_id: 'ENT-0035', project_id: 'proj_cpf', problem_summary: 'Database migrations must run before code deploy', source_type: 'vtt', team: 'Platform', confidence_score: 0.89, frequency_seen: 8, verification_status: 'verified', similarity: 0.85, capture_date: '2026-05-01', employee: 'Sneha Gupta' },
    ],
  },
  {
    answer: 'Stay away from /src/legacy/billing/ — it has undocumented side effects on shared database state and a refactor was already attempted and abandoned. If you need new billing behaviour, wrap around it instead, and talk to Arjun before touching anything there.',
    match: ['touch', 'legacy', 'avoid', 'fragile', 'codebase'],
    sources: [
      { entry_id: 'ENT-0031', project_id: 'proj_bil', problem_summary: 'Do not touch the legacy billing module', source_type: 'code', team: 'Backend', confidence_score: 0.96, frequency_seen: 2, verification_status: 'verified', similarity: 0.87, capture_date: '2026-04-20', employee: 'Arjun Sharma' },
    ],
  },
  {
    answer: 'There\u2019s a known race condition in the async payment pipeline: two concurrent webhook events can trigger the same payment handler and double-charge. The fix in place is a Redis distributed lock keyed on payment_id with a 30 second TTL — check the lock before processing.',
    match: ['race', 'payment', 'duplicate charge', 'webhook', 'async'],
    sources: [
      { entry_id: 'ENT-0039', project_id: 'proj_pay', problem_summary: 'Race condition in async payment pipeline', source_type: 'audio', team: 'Backend', confidence_score: 0.94, frequency_seen: 4, verification_status: 'verified', similarity: 0.9, capture_date: '2026-05-12', employee: 'Arjun Sharma' },
    ],
  },
]
...
export const getSuggestedQuestions = async () => {
  await delay(300)
  return [
    'Why does the billing export return an empty CSV?',
    'What is the deployment order for database migrations?',
    'Which parts of the codebase should I never touch?',
    'What are the known race conditions in the payment pipeline?',
  ]
}
...
export const getRecurringProblems = async () => {
  await delay(500)
  return [
    { entry_id: 'ENT-0041', problem_summary: 'Blank CSV on billing export', frequency_seen: 11, team: 'Backend', verification_status: 'verified', confidence_score: 0.91 },
    { entry_id: 'ENT-0035', problem_summary: 'Migration order on deploy', frequency_seen: 8, team: 'Platform', verification_status: 'verified', confidence_score: 0.89 },
...
export const getSystemHealth = async () => {
  await delay(400)
  return {
    status: 'healthy', llm: 'llama3.1:8b', ollama: 'http://localhost:11434',
    total_entries: 47, storage_used_mb: 128, storage_total_mb: 512,
    entries_by_project: [
      { project: 'BillingOS', count: 22 },
      { project: 'PaymentAPI', count: 14 },
      { project: 'CorePlatform', count: 11 },
    ],
    audit_log: [
