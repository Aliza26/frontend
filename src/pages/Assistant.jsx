...
function AgentMessage({ msg, employeeId }) {
  return (
    <div className="chat-row chat-row-agent fade-in-up">
      <div className="chat-avatar chat-avatar-logo">
        <KTHubLogo size={32} transparent variant="light" />
      </div>
      <div className="chat-bubble-col">
        ...
        {msg.sources?.length > 0 && (
          <div className="chat-citations">
            <span className="chat-citations-label">
              Based on {msg.sources.length} thing
              {msg.sources.length > 1 ? "s" : ""} the team knows
            </span>
            {msg.sources.map((c, i) => (
              <CitationCard
                key={c.entry_id}
        ...
        {msg.sources?.length === 0 && (
          <p className="chat-no-sources">
            <i className="ti ti-info-circle" aria-hidden="true" /> Nothing
            captured on this yet — worth asking your tech lead, and check back
            once more entries land.
          </p>
        )}
      </div>
    </div>
...
export default function Assistant() {
  ...
  useEffect(() => {
    api.getSuggestedQuestions().then(setSuggestions);
  }, []);
  useEffect(() => {
  ...
  return (
    <div className="assistant-page">
      <div className="assistant-sidebar">
        ...
        <p className="assistant-sidebar-sub">
          Everything the team has captured so far — go ahead and ask.
        </p>
        ...
        <div className="assistant-legend">
          <span className="assistant-legend-title">
            Where answers come from
          </span>
          {[
            ["coral", "Voice notes"],
            ["periwinkle", "Written notes"],
            ["pink", "Code comments"],
            ["teal", "Meetings"],
            ["amber", "Documents"],
          ].map(([color, label]) => (
      ...
      <div className="assistant-chat">
        <div className="assistant-chat-header">
          ...
          <div className="assistant-chat-title">
            <span>KTChat</span>
            <span className="assistant-chat-sub">
              kthub-llm · 100% on-premise · cited answers
            </span>
          </div>
        ...
        <div className="assistant-chat-messages">
          {messages.length === 0 && (
            <div className="assistant-welcome fade-in-up">
              ...
              <h2>Hey {firstName} 👋</h2>
              <p>
                Ask me anything about the codebase — I'll pull from what your
                team has already figured out, with sources attached.
              </p>
            </div>
          )}
            ...
            <div className="chat-row chat-row-agent fade-in-up">
              ...
              <div className="loading-pulse">
                ...
                <span className="loading-pulse-text">
                  Looking through what the team knows…
                </span>
              </div>
            </div>
          )}
