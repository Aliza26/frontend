...
const PROJECTS = [
  { id: 'proj_bil', name: 'BillingOS' },
  { id: 'proj_pay', name: 'PaymentAPI' },
  { id: 'proj_cpf', name: 'CorePlatform' },
]

export default function Signup() {
  const [form, setForm] = useState({ name: '', email: '', password: '', team: '', role_type: '', project: '' })
  ...
  return (
    <div className="auth-page">
      <div className="auth-illustration">
        <div className="auth-illustration-card pop-in"><i className="ti ti-users-group" aria-hidden="true" /></div>
        <h1 className="auth-illustration-title">
          Join the team — <span className="auth-highlight">start day one with everything they knew</span>
        </h1>
        <div className="auth-feature-list">
          {[
            ['ti-microphone', 'Speak a fix, it gets captured in seconds'],
            ['ti-message-dots', 'Ask anything, get cited answers from real teammates'],
            ['ti-lock', 'Everything stays on this machine. Nothing leaves.'],
          ].map(([icon, text]) => (
            <div key={text} className="auth-feature-row">
              <div className="auth-feature-icon"><i className={`ti ${icon}`} aria-hidden="true" /></div>
              <span>{text}</span>
            </div>
          ))}
        </div>
      ...
      <div className="auth-form-side">
        <div className={`auth-form-card auth-form-card-wide card ${shake ? 'shake' : ''}`}>
          <div className="auth-logo-mobile">
            <div className="auth-logo-mark"><i className="ti ti-bulb" aria-hidden="true" /></div>
            <span>MemoryOS</span>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <input className={`input ${errors.name ? 'auth-input-error' : ''}`}
              placeholder="Priya Patel" value={form.name} onChange={(e) => set('name', e.target.value)} />
            {errors.name && <span className="auth-field-error">{errors.name}</span>}

            <label className="auth-label">Email</label>
            <input className={`input ${errors.email ? 'auth-input-error' : ''}`}
              placeholder="you@tata.com" value={form.email} onChange={(e) => set('email', e.target.value)} />
            {errors.email && <span className="auth-field-error">{errors.email}</span>}

            <label className="auth-label">Password</label>
            <div className="auth-password-wrap">
              <input type={showPw ? 'text' : 'password'} className={`input ${errors.password ? 'auth-input-error' : ''}`}
                placeholder="At least 6 characters" value={form.password} onChange={(e) => set('password', e.target.value)} />
            ...
            <div className="auth-row-3">
              <div>
                ...
                <select className={`input ${errors.team ? 'auth-input-error' : ''}`}
              ...
              <div>
                ...
                <select className={`input ${errors.role_type ? 'auth-input-error' : ''}`}
              ...
              <div>
                ...
                <select className={`input ${errors.project ? 'auth-input-error' : ''}`}
